...
void process_HELLO_msg(int sock)
{
  ...
  oc = htons(MSG_HELLO_RP);
  hello_rp.opcode = oc;
  strcpy(hello_rp.msg, "Hello World");  //omplir msg amb la cadena "hello world"
  ...
  struct conn_any_hdr * conn_any_msg)
{
  ...
  unsigned short port;
  unsigned short err_code = 80; // si deixem el -1 peta, no sabem perque pero no iguala a -1 en el if de no error. D'aquesta manera ho fa correctament.
  
  memset(error_reply, 0, MAX_REPLY_SIZE);
...
int main(int argc, char* argv[])
{
  ...
  listen ( s, SWITCH_PORTS);
  int pid;
 
  
  while(1) {
    
    con_sock = accept (s, (struct sockaddr*)&client_addr, &client_addrlen);
    pid=fork();
