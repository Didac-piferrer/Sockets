...
{
  send_parameterless_msg(sock,OP_FINISH);
  close(sock);
  exit(0);     
}
...
void process_connect_to_operation(int sock)
{
  ...
  connect_to.port = htons(get_port_from_stdin());
  connect_to.opcode = htons(MSG_CONN_TO);
  set_MAC_from_stdin(connect_to.mac);
  send(sock, &connect_to, sizeof(struct conn_to_hdr),0);
...
void process_connect_to_any_operation(int sock)
{
  ...
  if(ntohs(reply.opcode) == MSG_CONN_ANY_RP)
  {
    printf("%s %hu\n\n",CONN_ANY_TXT_MSG, ntohs(reply.port) );
  }
  else
  {
...
void process_free_operation(int sock)
{
  ...
  p=(get_port_from_stdin());
  free_msg.port=htons(p);
...
void show_boolean_operation_feedback(char *reply)
{
  ...
  {
    error_code = ldshort(reply+2); //obtenir el codi d'error del missatge
...
{ 
  ...
  // declarar les variables per crear els sockets.
  int s;
  ...
  struct sockaddr_in cliente_connect;
  s = socket (PF_INET,SOCK_STREAM,IPPROTO_TCP);

// setting up the socket for communication
  cliente_connect.sin_family = AF_INET;
  cliente_connect.sin_port = htons(port);
  setaddrbyname(&cliente_connect, host);

  connect (s,(struct sockaddr*)&cliente_connect,sizeof(cliente_connect));
